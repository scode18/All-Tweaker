...
def execute():
    for checkbox_name, checkbox_var in checkboxes.items():
        if checkbox_var.get():
            ...
            subprocess.call(f'tweaks\\"{checkbox_name}.reg"', shell=True)
...
'Очистка': ['WinClean', 'Включить гибернацию', 'Включить зарезервированное хранилище', 'Запуск меню очистки', 'Запуск очистки', 'Отключить гибернацию', 'Отключить зарезервированное хранилище', 'Отменить сжатие системы', 'Очистить Windows v2.0', 'Очистить Windows', 'Очистить автозапуск в реестре', 'Очистить временные файлы и кэш иконок', 'Очистить кэш', 'Очистка и оптимизация дисков', 'Очистка кэша обновлений', 'Очистка папок Temp', 'Очистка файла подкачки', 'Очистка хранилища WinSxS', 'Проверить сжатие системы', 'Сбросить счетчик изображений', 'Сжать систему', 'Удалить временные файлы от Adamx', 'Удалить временные файлы', 'Удалить кеш Центра обновления Windows', 'Удалить лог файлы', 'Удалить скачанные файлы обновлений Windows', 'Уменьшить размер файла гибернации', 'Хардкор чистка 1', 'Хардкор чистка 2', 'Хардкор чистка 3'],
