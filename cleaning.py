...
for dir in os.listdir("tweaks"):
    ...
    if os.path.isdir(path) and dir not in ["База", "Исправление проблем", "Кастомизация", "Обновления", "Оптимизация", "Оптимизация YouTube", "Остальное", "Очистка", "Поддержка", "Приватность", "Программы", "Удалить приложения Microsoft", "Электропитание"]:
        shutil.rmtree(path)
    elif os.path.isfile(path) and dir not in ["База", "Исправление проблем", "Кастомизация", "Обновления", "Оптимизация", "Оптимизация YouTube", "Остальное", "Очистка", "Поддержка", "Приватность", "Программы", "Удалить приложения Microsoft", "Электропитание", "make.py", "tabs.py"]:
        os.remove(path)
